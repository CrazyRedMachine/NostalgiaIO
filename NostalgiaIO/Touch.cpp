...
Touch_::Touch_(void) : PluggableUSBModule(1, 1, epType) {
   epType[0] = EP_TYPE_INTERRUPT_IN;
      PluggableUSB().plug(this);
    }

    int Touch_::getInterface(byte* interfaceCount) {
    ...
    int Touch_::getDescriptor(USBSetup& setup)
    {
    ...
    bool Touch_::setup(USBSetup& setup)
    {
    ...
    uint8_t Touch_::getShortName(char *name)
    {
      name[0] = 'N';
      name[1] = 'O';
      name[2] = 'S';
...
int Touch_::send(uint8_t identifier) {
...
/* CUSTOM NOST FUNCTIONS */
    ...
    uint8_t Touch_::getLightMode(){
      return lightMode;
    }
    
    unsigned long Touch_::getLastHidUpdate(){
      return lastHidUpdate;
    }
    
    void Touch_::setLightMode(uint8_t mode){
    ...
    color_t Touch_::color_reactive(uint8_t button)
    {
    ...
    color_t Touch_::color_interlace(uint8_t button)
    {
...
#define NBSHADES 28
#define WAVESPEED 40
    color_t Touch_::color_rainbow(uint8_t button)
    {
...
#define WAVESPEED 40
    color_t Touch_::color_chase(uint8_t button)
    {
      static unsigned long startTime = millis();
      uint16_t angle;
...
#define WAVESPEED 100
    color_t Touch_::color_breath(uint8_t button)
    {
...
#undef WAVESPEED
    ...
    void Touch_::updateLeds(bool hid, color_t (*func)(uint8_t)){
      if (hid)
    ...
    uint8_t* Touch_::getButtonsState(){
      return buttonsState;
    }
#endif
